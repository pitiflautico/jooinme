<x-app-layout>
    <x-slot name="header">
        <h2 class="mb-0">{{ __('Editar Conversación') }}</h2>
    </x-slot>
    ...
    <x-backend.card>
        <form method="POST" action="{{ route('conversations.update', $conversation) }}">
            ...
            <div class="mb-4">
                ...
                <x-backend.text-input type="text" id="title" name="title" value="{{ old('title', $conversation->title) }}" required maxlength="255" placeholder="Ej: Club de Lectura Mensual: Ciencia Ficción" />
            ...
            <div class="mb-4">
                ...
                <x-backend.textarea id="description" name="description" required rows="4" placeholder="Describe tu conversación...">{{ old('description', $conversation->description) }}</x-backend.textarea>
            ...
            <div class="row g-3 mb-4">
                ...
                <div class="col-md-6">
                    ...
                    <x-backend.text-input type="number" id="max_participants" name="max_participants" value="{{ old('max_participants', $conversation->max_participants) }}" required min="2" max="100" />
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-6">
                    ...
                    <x-backend.text-input type="datetime-local" id="starts_at" name="starts_at" value="{{ old('starts_at', $conversation->starts_at?->format('Y-m-d\TH:i')) }}" />
                    <x-backend.input-error field="starts_at" />
                </div>

                <div class="col-md-6">
                    <x-backend.input-label for="ends_at" value="Fecha de fin (opcional)" />
                    <x-backend.text-input type="datetime-local" id="ends_at" name="ends_at" value="{{ old('ends_at', $conversation->ends_at?->format('Y-m-d\TH:i')) }}" />
            ...
            <div id="locationFields" style="display: none;">
                <div class="mb-3">
                    ...
                    <x-backend.text-input type="text" id="location" name="location" value="{{ old('location', $conversation->location) }}" placeholder="Ciudad, país" />
                    <x-backend.input-error field="location" />
                </div>

                <div>
                    <x-backend.input-label for="location_details" value="Detalles del lugar" />
                    <x-backend.textarea id="location_details" name="location_details" rows="2" placeholder="Dirección exacta, referencias...">{{ old('location_details', $conversation->location_details) }}</x-backend.textarea>
            ...
            <div class="mb-3">
                <div class="form-check">
                    <input type="checkbox" name="allow_chat" value="1" {{ old('allow_chat', $conversation->allow_chat) ? 'checked' : '' }} class="form-check-input" id="allow_chat">
            ...
            <div class="mb-3">
                <div class="form-check">
                    <input type="checkbox" name="allow_recording" value="1" {{ old('allow_recording', $conversation->allow_recording) ? 'checked' : '' }} class="form-check-input" id="allow_recording">
            ...
            <div class="mb-4">
                <div class="form-check">
                    <input type="checkbox" name="require_approval" value="1" {{ old('require_approval', $conversation->require_approval) ? 'checked' : '' }} class="form-check-input" id="require_approval">
